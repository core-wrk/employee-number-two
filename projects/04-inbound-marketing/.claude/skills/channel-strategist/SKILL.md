...
- `global/context/icp.md` — **read this carefully**, especially the "Where to Find Them" section. This is the strongest evidence you have for which channels the buyer actually inhabits. If the founder wants to invest in a channel that does not appear in this section, that is a red flag worth surfacing.
- `global/context/brand-voice.md` — to understand which channels the brand voice can sustain (a precise, deal-grounded voice will not survive on TikTok; a casual, story-led voice will not survive on a technical research blog)
- `global/context/startup-hypothesis.md` — for context on the live questions the inbound effort should not over-claim
- `global/context/founder-profile.md` — **read this carefully**; channel choices have to be sustainable for this specific founder. A founder who hates being on camera should not be running YouTube as a primary channel, no matter how well it might fit the ICP.
- `projects/02-brand-and-marketing/outputs/messaging-framework.md` — if it exists. The messaging pillars become the topical backbone for whichever channels you choose.

**If `brand-voice.md` or `icp.md` does not exist, stop immediately.** Tell the founder plainly:

> "I can't decide channels yet — I need these files first:
> - `global/context/brand-voice.md` (produced by Project 02's `brand-voice-developer`)
> - `global/context/icp.md` (produced by Project 01's `icp-builder`)
>
> Channel strategy without brand voice and ICP is just guessing. Please complete the prior projects and come back."

Also load references:
- `references/channel-evaluation-rubric.md` — internal scoring rubric: ICP fit, founder voice fit, time-to-signal, compounding value, capacity cost
- `references/channel-archetypes.md` — internal reference describing the strengths and weaknesses of LinkedIn (company and personal), Reddit, YouTube, blog/SEO, technical reports, podcasts, and communities — with examples of when each works and when it does not
...
### Phase 3: Channel-by-channel evaluation
...
1. **ICP fit** — Where in `icp.md` is the evidence the buyer is here? (1–5 scale, where 5 = "named in 'Where to Find Them' with multiple specific communities")
2. **Founder voice fit** — Can the brand voice (from `brand-voice.md`) sustain this channel? Can the founder personally show up here without burning out? (1–5)
3. **Time-to-signal** — How long until the founder will know whether the channel is working? (Reddit: days. LinkedIn: weeks. SEO blog: months.)
4. **Compounding value** — Does old content keep working, or does it die in 48 hours? (5 = compounds; 1 = ephemeral)
5. **Capacity cost** — How many hours per week to do this well, not just to dabble?

Do not show the rubric to the founder. Use it internally to keep the conversation structured.
...
- Why this channel (one or two sentences tying it back to `icp.md`)
- Cadence (specific: "2 LinkedIn text posts per week, 1 long-form per month," not "regularly")
- Format mix (which of the writer skills will produce content for this channel)
- The 60-day kill criterion (a specific signal — "200 qualified visitors" or "10 substantive replies" or "5 waitlist signups attributed to this channel")
- Who owns it (founder, contractor, future hire)

For each channel **not** committed to, capture a one-line reason — so future-self knows why the choice was made and does not relitigate it on a bad day.
...
### Phase 6: Capacity budget reconciliation

Do the math one more time:
- Channel A: X posts per week × Y hours per post = total weekly hours
- Channel B: similar
- Channel C: similar
- Sum = total inbound time per week
...
- 2–3 primary channels committed to (no more)
- Every committed channel has evidence from `icp.md` cited explicitly
- Every committed channel has a cadence in concrete numbers (posts per week, time per post)
- Every committed channel has a 60-day kill criterion that is measurable
- Every committed channel has a capacity cost in hours per week
- The total capacity cost fits inside the founder's stated weekly budget
- At least one channel "not committed to" is captured with a one-line reason
- The founder has been pushed back on at least once if they tried to commit to more than 3 channels

If any of these are missing, continue the conversation. Do not write a strategy that cannot be executed.
...
1. Tell the founder you are ready to write the channel strategy
2. Show a complete preview of the file content
3. Ask if anything needs to change
4. Once approved, write the file to `projects/04-inbound-marketing/outputs/marketing-channel-strategy.md`
5. Confirm the file was written
6. Tell the founder the next step is `content-strategist`, which will turn the channel strategy into a concrete content calendar

## What Not To Do

- Do not write the strategy if the founder has committed to more than 3 channels — push back first
- Do not produce a strategy whose total capacity cost exceeds the founder's stated weekly budget — cut cadence or cut channels
- Do not commit to a channel that does not appear in `icp.md`'s "Where to Find Them" section unless the founder can articulate a specific reason their buyer is there anyway
- Do not let the founder commit to a channel that conflicts with their voice or working style — channel sustainability matters more than channel theory
- Do not skip the kill criteria — naming them up front is the only thing that makes cutting a channel emotionally possible later
- Do not write the file without showing a preview and getting explicit approval
- Do not produce vague cadence guidance ("post regularly") — every cadence is in concrete numbers
- Do not invent channels the ICP does not support; do not push the founder toward TikTok or Instagram if their buyer is a 45-year-old VP of Sales Enablement
- Do not pretend channels are equivalent — be honest about which compound and which die in 48 hours
